...
public class UserController : ControllerBase
{
    ...
    /// This endpoint receives the Kongregate user ID, username, and game auth token,
    /// verifies the token with Kongregate's API, and creates or finds the corresponding user account.
    /// </summary>
    [HttpPost("kongregate-login")]
    public async Task<IActionResult> KongregateLogin([FromBody] KongregateLoginRequest request)
    {
        if (request.KongregateUserId <= 0)
        {
            return BadRequest("Kongregate user ID is required.");
        }

        if (string.IsNullOrEmpty(request.GameAuthToken))
        {
            return BadRequest("Game auth token is required.");
        }

        // Verify the token with Kongregate's API
        var kongregateApiKey = _configuration["Kongregate:ApiKey"];
        if (string.IsNullOrEmpty(kongregateApiKey))
        {
            return StatusCode(500, "Kongregate API key not configured.");
        }

        using var httpClient = new HttpClient();
        var verifyUrl = $"https://api.kongregate.com/api/authenticate.json?user_id={request.KongregateUserId}&game_auth_token={Uri.EscapeDataString(request.GameAuthToken)}&api_key={Uri.EscapeDataString(kongregateApiKey)}";
        ...
        try
        {
            ...
            {
                return Unauthorized("Kongregate authentication failed. Invalid token.");
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Kongregate auth verification failed: {ex.Message}");
            return StatusCode(500, "Failed to verify Kongregate authentication.");
        }

        var kongregateUserIdStr = request.KongregateUserId.ToString();
        ...
        if (User.Identity.IsAuthenticated)
        {
            ...
            if (currentUser != null)
            {
                // If user already has a Kongregate ID linked, check if it matches
                if (!string.IsNullOrEmpty(currentUser.KongregateUserId))
                {
                    if (currentUser.KongregateUserId == kongregateUserIdStr)
                    {
                        // Already linked to this Kongregate account - just return success
                        var token = GenerateJwtToken(currentUser);
                        return Ok(new { userId = currentUser.Id, linked = true, token });
                    }
                    else
                    {
                        // Different Kongregate account - don't allow re-linking
                        return BadRequest("This account is already linked to a different Kongregate account.");
                    }
                }
                
                // Link the Kongregate account to the current user (guest upgrade scenario)
                currentUser.KongregateUserId = kongregateUserIdStr;
                if (currentUser.IsGuest)
                {
                    currentUser.IsGuest = false;
                    // Set display name from Kongregate username if provided
                    if (!string.IsNullOrEmpty(request.KongregateUsername))
                    {
                        currentUser.DisplayName = request.KongregateUsername;
                        currentUser.UserName = await GetUniqueUsernameAsync(request.KongregateUsername, null);
                    }
                }
                await _userManager.UpdateAsync(currentUser);
                await _signInManager.RefreshSignInAsync(currentUser);
                var linkedToken = GenerateJwtToken(currentUser);
                return Ok(new { userId = currentUser.Id, linked = true, token = linkedToken });
            }
        }

        // Try to find existing user by Kongregate ID
        var existingUser = await _context.Users
            .FirstOrDefaultAsync(u => u.KongregateUserId == kongregateUserIdStr);

        if (existingUser != null)
        {
            // Found existing Kongregate user - sign them in
            await _signInManager.SignInAsync(existingUser, isPersistent: true);
            var existingToken = GenerateJwtToken(existingUser);
            return Ok(new { userId = existingUser.Id, isNew = false, token = existingToken });
        }

        // Create a new user for this Kongregate account
        var username = !string.IsNullOrEmpty(request.KongregateUsername) 
            ? await GetUniqueUsernameAsync(request.KongregateUsername, null)
            : await GetUniqueUsernameAsync($"Kong_{kongregateUserIdStr}", null);
        
        var newUser = new User
        {
            UserName = username,
            KongregateUserId = kongregateUserIdStr,
            DisplayName = request.KongregateUsername ?? $"Kong_{kongregateUserIdStr}",
            IsGuest = false,
            SourceIp = HttpContext.Connection.RemoteIpAddress?.ToString(),
            GuessEnergy = GameConfig.Current.MaxGuessEnergy,
            ClueEnergy = GameConfig.Current.MaxClueEnergy,
        ...
        if (createResult.Succeeded)
        {
            ...
            return Ok(new { userId = newUser.Id, isNew = true, token = newToken });
        }
...
{
    public long KongregateUserId { get; set; }
    public string? KongregateUsername { get; set; }
